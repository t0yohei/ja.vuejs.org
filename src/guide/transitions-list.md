...
それでは、例えば、`v-for` のように同時にレンダリングしたいリストのアイテムがある場合はどうすればよいでしょう？この場合は、`<transition-group>` コンポーネントを使うことができます。例を詳しく見ていく前に、このコンポーネントについて知っておくべき幾つかの重要なことをあげておきましょう。

- `<transition>` と異なり、実際に要素を描画する: `<span>` がデフォルトです。この要素は、`tag` 属性で変えることができます。
- [トランジションモード](/guide/transitions-enterleave#transition-modes) は利用できません。もはや相互に排他的な要素を交互に利用する事が無いためです。
